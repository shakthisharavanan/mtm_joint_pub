...
public:
	...
	{
		// switch_value = 1; 
		student_joints_sub = n.subscribe<sensor_msgs::JointState>("/mtm_student/joint/states", 100, &mtm_teacher::joints_cb, this);
		// teacher_switch_sub = n.subscribe<std_msgs::Int32>("/mtm_teacher_switch", 100, &mtm_student::switch_cb, this);

		outer_yaw_pub = n.advertise<std_msgs::Float64>("/mtm_teacher/right_outer_yaw_joint/SetPositionTarget", 100);
		shoulder_pub = n.advertise<std_msgs::Float64>("/mtm_teacher/right_shoulder_pitch_joint/SetPositionTarget", 100);
		elbow_pub = n.advertise<std_msgs::Float64>("/mtm_teacher/right_elbow_pitch_joint/SetPositionTarget", 100);
		platform_pub = n.advertise<std_msgs::Float64>("/mtm_teacher/right_wrist_platform_joint/SetPositionTarget", 100);
		pitch_pub = n.advertise<std_msgs::Float64>("/mtm_teacher/right_wrist_pitch_joint/SetPositionTarget", 100);
		yaw_pub = n.advertise<std_msgs::Float64>("/mtm_teacher/right_wrist_yaw_joint/SetPositionTarget", 100);
		roll_pub = n.advertise<std_msgs::Float64>("/mtm_teacher/right_wrist_roll_joint/SetPositionTarget", 100);
		switch_pub = n.advertise<std_msgs::Int32>("/mtm_teacher_switch", 100);
	...
	{	
		...
		cv::createTrackbar("Take Control", "Joints", &slider, 1);
		...
		cv::setTrackbarPos("Take Control", "Joints", 1);


		while (ros::ok())
		{
			ros::spinOnce();
			switch_value.data = cv::getTrackbarPos("Take Control", "Joints");
			if (switch_value.data == 1)
			{	
				...
				switch_pub.publish(switch_value);
...
{
	ros::init(argc, argv, "mtm_teacher");
	mtm_teacher teach;
	try
		{
			teach.spin();
